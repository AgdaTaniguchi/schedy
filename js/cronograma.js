...
function adicionarTr(novoHorario, horario){
    ...
    var diaSemana = ['domingo', 'segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado'];

    for(let i = 0; i < 7; i++) {
        if($(window).width() < 1366){
            if($(`input[type='checkbox']`).eq(i).is(':checked')){
                if($(window).width() < 1366 && diaSemana[i] == $('#selecionaDia').val()){
                    novaTr.next('tr').append(`<td><div class="evento" style="background-color: #${$('.selecionado').attr("id")}">${$('#evento').val()}</div></td>`);
                }
                else{
                    novaTr.next('tr').append(`<td class="none"><div class="evento" style="background-color: #${$('.selecionado').attr("id")}">${$('#evento').val()}</div></td>`);
                }
            }
            else{
                if($(window).width() < 1366 && diaSemana[i] == $('#selecionaDia').val()){
                    ...
                    novaTr.next('tr').append('<td class="none">-</td>');
                }
            }
        }
        else{
...
$(document).ready(function(){
    $('.btnAdicionarEvento').css('top', ($('footer').position().top - 100) + 'px');
    ...
    if($(window).width() < 801){
        $('.btnAdicionarEvento').css('position', 'absolute');
    }
    else{
        $('.btnAdicionarEvento').css('position', 'fixed');
    }
    ...
    $(window).resize(function(){
        ...
        // Posicionar o botão em cima do footer
        $('.btnAdicionarEvento').css('top', ($('footer').position().top - 100) + 'px');
